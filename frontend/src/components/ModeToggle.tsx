...
function ModeToggleInner({ value, onChange, disabled = false }: ModeToggleProps): JSX.Element {
  return (
    <div
      ...
      className="inline-flex shrink-0 rounded-full bg-surface-raised shadow-elev-1"
    >
      {MODES.map((m) => {
        ...
        return (
          <button
            ...
            className={
              "flex min-h-11 items-center justify-center rounded-full px-3 text-xs font-medium transition active:scale-95 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent " +
