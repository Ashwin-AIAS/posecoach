...
def test_threshold_boundary_below_excluded() -> None:
    kp = np.random.rand(17, 2).astype(float)
    kp_conf = np.ones(17, dtype=float) * (ANGLE_CONF_THRESHOLD - 0.01)
    angles = compute_angles(kp, kp_conf)
...
def test_threshold_boundary_at_included() -> None:
    kp = np.random.rand(17, 2).astype(float)
    kp_conf = np.ones(17, dtype=float) * ANGLE_CONF_THRESHOLD
    angles = compute_angles(kp, kp_conf)
...
def test_all_low_confidence_returns_position_cue(exercise: str) -> None:
    ...
    # No joint clears the gate → explicit insufficient_confidence status, not a
    # silent 0.0 that would read as terrible form (P13).
    assert result.status == STATUS_INSUFFICIENT_CONFIDENCE
    assert not result.joint_scores
